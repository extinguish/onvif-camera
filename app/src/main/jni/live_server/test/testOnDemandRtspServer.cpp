...
/// testOnDemandRtspServer 本身是基于live555实现的最基础的rtsp server
...
// 这里的announceStream方法是为了将新创建的RtspServer stream广播出去
// 因为live555的实现本身支持rtsp的最重要的一种业务场景，即视频会议系统
// 因此每当有一个新的视频流可用时,就是通过announceStream来将新流广播出去
// 通知到当前会议当中的所有其他成员(不过视频会议本身基本上都是基于"组播"协议实现的)
  ...
  // access to the server.
    ...
    // A H.264 video elementary stream:
    {
        ...
        ServerMediaSession *sms
                = ServerMediaSession::createNew(*env, streamName, streamName,
                                                descriptionString);
        // 这里调用了addSubssession之后，live555内部会负责进行视频的解码操作
        sms->addSubsession(H264VideoFileServerMediaSubsession
                           ::createNew(*env, inputFileName, reuseFirstSource));
        rtspServer->addServerMediaSession(sms);

        // 此时客户端就可以根据视频流的rtsp地址来调用播放该流了
        announceStream(rtspServer, sms, streamName, inputFileName);
    }
